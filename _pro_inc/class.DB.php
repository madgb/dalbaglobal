...
class DB{
	...
	public function log_db_errors($error, $query){
		...
		if(!defined('DISPLAY_DEBUG') || (defined('DISPLAY_DEBUG') && DISPLAY_DEBUG)){
			echo "<script>alert('" . $query . "'); history.back(-1);</script>";
		}
	}

	public function __construct($db_host, $db_user, $db_pass, $db_name, $dbport = 3306){
		// mb_internal_encoding('UTF-8');
		// mb_regex_encoding('UTF-8');
		mysqli_report(MYSQLI_REPORT_STRICT);

		try{
			$this->link = new mysqli($db_host, $db_user, $db_pass, $db_name, $dbport);
	...
	public function htmlfilter($data){
		if(!is_array($data)){
			...
			$data = array_map([$this, 'htmlfilter'], $data);
		}
		return $data;
	}
	...
	/**
	 * Extra function to htmlfilter when only mysqli_real_escape_string is needed
	 * @access public
	 *
	 * @param mixed $data
	 *
	...
	/**
	 * Normalize sanitized data for display (reverse $database->htmlfilter cleaning)
	 *
	 * Example usage:
	...
	public function get_row($query, $object = false){
		...
		if($this->link->error){
			...
			$r = (!$object) ? $row->fetch_array() : $row->fetch_row();
			return $r;
		}
	}
	...
	public function get_results($query, $object = false){
		...
		if($this->link->error){
			...
			$row = [];
			$row['cnt'] = $results->num_rows;

			while($r = (!$object) ? $results->fetch_assoc() : $results->fetch_row()){
				$row['result'][] = $r;
			}
			return $row;
		}
	 ...
	 *
	 */
	public function insert($table, $variables = [], $view = "N"){
	 ...
	 *
	 */
	public function update($table, $variables = [], $where = [], $limit = '', $view="N"){
		self::$counter++;
