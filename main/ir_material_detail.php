...
$query = " SELECT b_idx, subject, bbs_cd, content, w_dt, w_year, url FROM tb_board_info WHERE b_idx = '$b_idx' ";
...
<main class="main">
        <div class="section_2 ir_section">
          <div class="ir_inner detail_page">
            <span class="gray_badge"><?=ucfirst(substr($post['bbs_cd'], 3))?></span>
            <h1 class="title"><?=$post['subject']?></h1>
            <p class="day"><?=date('Y.m.d', strtotime($post['w_dt'])) ?></p>
            <div class="content">
              <?=$post['content']?>
            </div>
